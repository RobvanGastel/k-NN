...
p = 8
info = np.loadtxt(r'results/QE/minkowski_test_LOOCV.txt', delimiter=',')
...
while True:
    ...
    if last_index == len(y_test):
        print('DONE!')
        break
    print('last_index', last_index, '/', len(X_test))

    y_hat = cls.predict(
        X_test[last_index+1:last_index+101, :],
        'minkowski',
        LOOCV=True,
        p=p
    )
    ...
    for k in range(1, 21):
        info = np.vstack((
            ...
            np.array([
                last_index+100,
                np.sum(y_hat[:, k-1] != y_test[last_index+1:last_index+101]),
                k,
                p
            ])
        ))

    write_results(info, 'minkowski', small=False, train=False, LOOCV=True)